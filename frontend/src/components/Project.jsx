...
function Project() {
  ...
  const addProjectEpisode = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios_create.post(
        `/project/createEpi/${projectId}`,
        episode
      );
      getProjectEpisode(projectId);
      setLoading(false);
      toast({
        title: "Epesode Created.",
  ...
  const deleteProjectEpisode = async (episodeId) => {
    try {
      setLoading(true);
      const res = await axios_create.delete(`/project/episode/${episodeId}`);
      getProjectEpisode(projectId);
      setLoading(false);
      toast({
        title: "Epesode Deleted.",
  ...
  const updateProjectEpisode = async () => {
    try {
      ...
      const res = await axios_create.patch(
        `/project/episode/${episodeData[indexValue]._id}`,
        { description: newtext }
      );
      getProjectEpisode(projectId);
      setEdit(false);
                              ...
                              <>
                                <tr className="border-b border-[#999999] ">
                                  ...
                                  <td className="py-2 px-4 text-left">
                                    {dateConvertor(episode.createdAt)}
                                  </td>
